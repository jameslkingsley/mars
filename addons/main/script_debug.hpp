...
//#define ENABLE_PERFORMANCE_COUNTERS
    ...
    #define CBA_fnc_addPerFrameHandler { _ret = [_this select 0, _this select 1, _this select 2, #function] call CBA_fnc_addPerFrameHandler; ISNILS(ACE_PFH_COUNTER,[]); ACE_PFH_COUNTER pushBack [[_ret, __FILE__, __LINE__], [_this select 0, _this select 1, _this select 2]];  _ret }

    #define CREATE_COUNTER(x) ISNILS(ACE_COUNTERS,[]); GVAR(DOUBLES(x,counter)) = []; GVAR(DOUBLES(x,counter)) set [0, QUOTE(GVAR(DOUBLES(x,counter)))];  GVAR(DOUBLES(x,counter)) set [1, diag_tickTime]; ACE_COUNTERS pushBack GVAR(DOUBLES(x,counter));
